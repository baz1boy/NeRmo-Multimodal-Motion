...
from sim_control import simModel as robot
...
class robotController(object):
	...
	def __init__(self):
		...
		self.init_pleg = np.array([[0.0325, -0.0060675, -0.050, 1], [-0.0325, -0.0060675, -0.050, 1],
								[0.0325, -0.0060675, -0.050, 1],[-0.0325, -0.0060675, -0.050, 1]])  #0.006075
		self.init_qdiff = np.array([20,90,0,20,90,0,20,90,0,20,90,0])
		...
		PI = np.pi
		self.stepNum = 200			# default period T=2s, 0.002x100
		self.cycle = 1
	...
	def getData(self):
		pos, sensordata = robot.getPos()
		gyro = gyro = sensordata[-3:]
	...
	def pryStance(self, pleg, move, angle, center):
		...
		q_data, pleg_cur = pos.pitchrollyaw(pleg, pbody, angle_cur, center)
	...
	def squeezingControl(self, index):
		...
		if index == 0:
			...
			p_squ = np.array([[0.0325, -0.02145, -0.02], [-0.0325, -0.02145, -0.02], 
							[0.0325, -0.02145, -0.02], [-0.0325, -0.02145, -0.02]])
			q0, q1, q2, q3 = pos.squeezing(p_squ, 1)

		elif index == 2:
			p_after_squ = np.array([[0.0325, 0.004, -0.042], [-0.0325, 0.004, -0.042], 
									[0.0325, 0.004, -0.042], [-0.0325, 0.004, -0.042]])
	...
	def squeezingnarrowControl(self, index):
		if index == 0:
			...
			p_adj2 = np.array([[0.0325, -0.0160675, -0.0374], [-0.0325, -0.0160675, -0.0374], 
								[0.0325, -0.0160675, -0.0374], [-0.0325, -0.0160675, -0.0374]])
			...
			p_adj5 = np.array([[0.0325, -0.0160675, -0.0374], [-0.0325, -0.0160675, -0.0374], 
								[0.0325, -0.0160675, -0.0374], [-0.0325, -0.0160675, -0.0374]])
	...
	def jumpControl(self, pleg, index):
		if index == 0:
			q_squat = p2q(pleg)
			q_out = pos.jumpMotion_0(q_squat)
			n = len(q_out)
			q_head = np.zeros((n, 2))
			q_data = np.hstack((q_head, q_out))
		elif index == 1:
			p_squat = np.array([[0.0325, -0.0060675, -0.035], [-0.0325, -0.0060675, -0.035], 
								[0.0325, -0.0060675, -0.035], [-0.0325, -0.0060675, -0.035]])	
			# p_squat = np.array([[0.0325, -0.021, -0.02], [-0.0325, -0.021, -0.02], 
			# 					[0.0325, -0.021, -0.02], [-0.0325, -0.021, -0.02]])
			q_out = pos.jumpMotion_1(pleg, p_squat)
			n = len(q_out)
			q_head = np.zeros((n, 2))
			q_data = np.hstack((q_head, q_out))
		elif index == 2:
			p_squat = np.array([[0.0325, -0.0060675, -0.035], [-0.0325, -0.0060675, -0.035], 
								[0.0325, -0.0060675, -0.035], [-0.0325, -0.0060675, -0.035]])	
			q_out = pos.jumpMotion_2(pleg, p_squat)
			n = len(q_out)
			q_head = np.zeros((n, 2))
			q_data = np.hstack((q_head, q_out))
	...
	def surmountControl(self, pleg_cur, index):
		...
		if index == 1:
			...
			p3 = np.array([[0.0325, -0.02, -0.030], [-0.0325, -0.02, -0.030], 
			...
			p = np.array([[0.0325, -0.02, -0.030], [-0.0325, -0.02, -0.030], 
							[0.0325, 0.02, -0.080], [-0.0325, 0.02, -0.080]])		
			...
			p = np.array([[0.0325, 0.02, -0.060], [-0.0325, 0.02, -0.060], 
							[0.0325, 0.02, -0.080], [-0.0325, 0.02, -0.080]])
			# for i in range(2):
