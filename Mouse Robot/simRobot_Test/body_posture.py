...
def fallRecovery(pleg, q_init, index):
    ...
    if index==0:   ### robot in a side-lying position
        ...
        leg_add = 45
        q_move = np.array([0,0, servo1,servo2,-leg_add, servo1,servo2,-leg_add, servo1,servo2,-leg_add, servo1,servo2,-leg_add])
        t2 = 50     # 50 = 0.1s
        q_out2 = np.array([np.linspace(q_pos[i], q_move[i], t2) for i in range(14)]).T

        q_wait = np.vstack([q_move]*500)

        q_stand = np.array([0,0, servo1,servo2,0, servo1,servo2,0, servo1,servo2,0, 0,servo2,0])
        t3 = 400
        q_out3 = np.array([np.linspace(q_move[i], q_stand[i], t3) for i in range(14)]).T

        t4 = 200
        q_out4 = np.array([np.linspace(q_stand[i], q_init[i], t4) for i in range(14)]).T

        q_out = np.vstack((q_out1, q_out2, q_wait, q_out3, q_out4))
    elif index==1:
        ...
        q_p1_wait1 = np.vstack([q_pos]*200)

        q_headmove1 = np.array([0,tilt, 0,50,0, 0,50,0, 0,50,0, 0,50,0])
        t_p1_2 = 200
        q_p1_2 = np.array([np.linspace(q_pos[i], q_headmove1[i], t_p1_2) for i in range(14)]).T

        q_p1_wait2 = np.vstack([q_headmove1]*400)

        q_headmove2 = np.array([90,tilt, 0,50,0, 0,50,0, 0,50,0, 0,50,0])
        t_p1_3 = 200
        q_p1_3 = np.array([np.linspace(q_headmove1[i], q_headmove2[i], t_p1_3) for i in range(14)]).T

        q_p1_wait3 = np.vstack([q_headmove2]*400)

        q_headmove3 = np.array([90,tilt, 0,50,0, 0,50,20, 0,50,0, 0,50,20])
        t_p1_4 = 200
        q_p1_4 = np.array([np.linspace(q_headmove2[i], q_headmove3[i], t_p1_4) for i in range(14)]).T

        q_p1_wait4 = np.vstack([q_headmove3]*200)

        q_headmove4 = np.array([0,tilt, 0,50,0, 0,50,20, 0,50,0, 0,50,20])
        t_p1_5 = 200
        ...
        ###phase 2:: side lying to standing
        q_p2_wait1 = np.vstack([q_headmove5]*400)

        q_pos = np.array([0,0, 0,115,0, 0,115,0, 0,115,0, 0,115,0])
        t_p2_1 = 100
        q_p2_1 = np.array([np.linspace(q_headmove5[i], q_pos[i], t_p2_1) for i in range(14)]).T

        q_move = np.array([0,0, 0,115,-40, 0,115,-40, 0,115,-40, 0,115,-40])
        t_p2_2 = 50
        q_p2_2 = np.array([np.linspace(q_pos[i], q_move[i], t_p2_2) for i in range(14)]).T

        q_stand = np.array([0,0, 0,115,0, 0,115,0, 0,115,0, 0,115,0])
        t_p2_3 = 400
        q_p2_3 = np.array([np.linspace(q_move[i], q_stand[i], t_p2_3) for i in range(14)]).T

        q_p2_wait2 = np.vstack([q_stand]*200)

        t_p2_4 = 200
        q_p2_4 = np.array([np.linspace(q_stand[i], q_init[i], t_p2_4) for i in range(14)]).T
    elif index==2:
        ###phase 1:: from back-lying to side-lying
        q_pos = np.array([0,0, 0,50,0, 0,50,0, 0,50,0, 0,50,0])
        t_p1_1 = 100
        ...
        q_headmove3 = np.array([-90,tilt, 0,50,20, 0,50,0, 0,50,20, 0,50,0])
        t_p1_4 = 200
        q_p1_4 = np.array([np.linspace(q_headmove2[i], q_headmove3[i], t_p1_4) for i in range(14)]).T

        q_p1_wait4 = np.vstack([q_headmove3]*200)

        q_headmove4 = np.array([0,tilt, 0,50,20, 0,50,0, 0,50,20, 0,50,0])
        t_p1_5 = 200
        q_p1_5 = np.array([np.linspace(q_headmove3[i], q_headmove4[i], t_p1_5) for i in range(14)]).T
        ...
        q_out = np.vstack((q_p1_1, q_p1_wait1, q_p1_2, q_p1_wait2, q_p1_3, q_p1_wait3, q_p1_4, q_p1_wait4, q_p1_5, q_p1_wait5, q_p1_6,
                           q_p2_wait1, q_p2_1, q_p2_2, q_p2_3, q_p2_wait2, q_p2_4))
...
def squeezing(pleg, index):
    if index == 0:
        ...
        pleg_fore0 = np.array([[0.04, -0.01645, -0.02], [-0.04, -0.01645, -0.02]])
        pleg_foref = np.array([[0.04, -0.03245, -0.02], [-0.04, -0.03245, -0.02]])
...
def jumpMotion_0(q):
    ...
    q1[11] = 20
    t1 = 400
    q_out1 = np.array([np.linspace(q[i], q1[i], t1) for i in range(12)]).T

    q_wait = np.vstack([q1]*200)

    t2 = 40   # 50 = 0.1s
    q2 = q.copy()
    q_out2 = np.array([np.linspace(q1[i], q2[i], t2) for i in range(12)]).T

    q_out = np.vstack((q_out1, q_wait, q_out2))
    return q_out

def jumpMotion_1(p1, p2):
    q1 = calculateRobotservo(p1)
    q2 = calculateRobotservo(p2)
    t1 = 400
    q_out1 = np.array([np.linspace(q1[i], q2[i], t1) for i in range(12)]).T

    q_wait1 = np.vstack([q2]*200)
    ...
    p_jump[2][2] = -0.08
    p_jump[3][2] = -0.08
    q_jump = calculateRobotservo(p_jump)

    t2 = 40
    q_out2 = np.array([np.linspace(q2[i], q_jump[i], t2) for i in range(12)]).T

    q_wait2 = np.vstack([q_out2[-1]]*20)

    t3 = 40 
    q_out3 = np.array([np.linspace(q_out2[-1][i], q2[i], t3) for i in range(12)]).T

    q_out = np.vstack((q_out1, q_wait1, q_out2, q_wait2, q_out3))

    return q_out 

def jumpMotion_2(p1, p2):
    q1 = calculateRobotservo(p1)
    q2 = calculateRobotservo(p2)
    ...
    t1 = 400
    q_out1 = np.array([np.linspace(q1[i], q_squat1[i], t1) for i in range(12)]).T

    q_wait1 = np.vstack([q_squat1]*200)
    ...
    # t_abad1 = 20
    # q_abad1 = np.array([np.linspace(q_squat1[i], q_squat2[i], t_abad1) for i in range(12)]).T
    ...
    # t_abad2 = 20
    # q_abad2 = np.array([np.linspace(q_squat2[i], q_jump[i], t_abad2) for i in range(12)]).T
    
    t2 = 40
    q_out2 = np.array([np.linspace(q_squat1[i], q_jump[i], t2) for i in range(12)]).T

    q_wait2 = np.vstack([q_jump]*40)

    t3 = 100 
    q_out3 = np.array([np.linspace(q_jump[i], q2[i], t3) for i in range(12)]).T
